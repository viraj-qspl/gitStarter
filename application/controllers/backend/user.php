...
//        $this->load->model('area_model');
    ...
    function createRecordObj() {
        ...
        $recordObj->setId($id);
        $recordObj->setUserType($this->input->post('userType'));
        $recordObj->setEmail($this->input->post('email'));
        $recordObj->setPassword($this->input->post('password'));
        $recordObj->setFacebookId($this->input->post('facebookId'));
        $recordObj->setFirstName($this->input->post('firstName'));
        $recordObj->setLastName($this->input->post('lastName'));
        $recordObj->setDisplayName($this->input->post('displayName'));
        $recordObj->setGender($this->input->post('gender'));
        $recordObj->setBirthDate($this->input->post('birthDate'));
        $recordObj->setProfileImageFilename($this->input->post('profileImageFilename'));
        $recordObj->setAddress($this->input->post('address'));
        $recordObj->setLandmark($this->input->post('landmark'));
        $recordObj->setCountryId($this->input->post('countryId'));
        $recordObj->setStateId($this->input->post('stateId'));
        $recordObj->setCityId($this->input->post('cityId'));
        $recordObj->setAreaId($this->input->post('areaId'));
        $recordObj->setPincode($this->input->post('pincode'));
        $recordObj->setPhone($this->input->post('phone'));
        $recordObj->setCreatedDate(date("Y-m-d H:m:s")); //default current date
        $recordObj->setUpdatedDate("0000-00-00 00:00:00"); //default empty
        $recordObj->setLastLoginDate("0000-00-00 00:00:00"); //default empty
        $recordObj->setStatus(ACTIVE); //default active
    ...
    function setFormValidationRules($argViewArr) {
        ...
        $this->form_validation->set_rules('firstName', 'First Name', 'trim|required|min_length[' . NAME_MIN_LENGTH . ']|max_length[' . NAME_MAX_LENGTH . ']|xss_clean');
        $this->form_validation->set_rules('lastName', 'Last Name', 'trim|required|min_length[' . NAME_MIN_LENGTH . ']|max_length[' . NAME_MAX_LENGTH . ']|xss_clean');
