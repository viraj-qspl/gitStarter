...
class Common_method {
	...
	function getLimitOffsetAfterDelete($argStrIds, $argDataObj){
        ...
        //included in common_method as its common to both home/forgotPassword and api/forgotPassword
        function sendEmail($argObj)
        {	
            $CI =& get_instance(); //This is required as common is a custom class and not codeIgnitors controller or model				
            ...
            $CI->email->from($CI->config->item('fromEmail'), $CI->config->item('fromName'));
            $CI->email->to($argObj->getEmail()); 

            $CI->email->subject('New Password On User Request');

            $templateName = "forgotpassword";
            $templateData = array("email" => $argObj->getEmail(), "password" => $argObj->getPassword(), "user_id" => $argObj->getId()); 
            ...
            return $status;         //true or false based on email sent.
        }
